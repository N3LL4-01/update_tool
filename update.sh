...
APP_TITLE='Update Project Tool - by Nella'
...
gui_choose_project_actions () {
	dialog	--backtitle "${APP_TITLE}" \
					...
					--cancel-label 'Back' \
					--menu 'Choose action:'  ${SCREEN_HEIGHT_LO} ${SCREEN_WIDTH_LO} 3 \					
					1 'Update project' \
					2 'Composer update' 2> ${_TEMP}

	if [ ${?} -eq 0 ]; then
		ACTION=`cat ${_TEMP}`
		rm -f ${_TEMP}
		case ${ACTION} in
			1) gui_update_project;;
			2) gui_update_composer;;
		esac
	else
		gui_choose_main_action
	fi
